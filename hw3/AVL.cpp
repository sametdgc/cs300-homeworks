...
template <class Key, class Value>
int AVLSearchTree<Key, Value>::countUniqueWords(Node* node) {
    if (node == nullptr) {
        return 0;
    }
    // Traverse the left and right subtrees and count unique words recursively
    int leftCount = countUniqueWords(node->left);
    int rightCount = countUniqueWords(node->right);
    // Add 1 for the current node's key
    return leftCount + rightCount + 1;
}

// Implementation of uniqueWordCount function
template <class Key, class Value>
int AVLSearchTree<Key, Value>::uniqueWordCount() {
    return countUniqueWords(root); // Start counting from the root node
}